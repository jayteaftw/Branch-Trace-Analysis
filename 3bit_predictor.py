...
def threeBitPredictor(fileName):
    f = open(fileName, 'r')
    res = []
    ...
    for i in res:
        ...
        if i[1] == conv(pred): 
            correct += 1
            twoBit[idx] = min(pred+1, 3) if i[1] == 'T' else max(-2, pred-1)
        else:
            incorrect += 1
    ...
    print(f'Dynamic 3 bit Prediction:, total:{len(res)}, Miss Prediction: {round((len(res) - correct ) / len(res), 3)}')
